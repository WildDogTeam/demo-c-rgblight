...
- (void)setUp
{
    ...
    self.editable = YES;
    self.moreOptionsButtonTitle = @"gengming";
    self.deleteButtonTitle = @"shanchu";
...
{
    return CGRectGetWidth(self.deleteButton.frame) + CGRectGetWidth(self.moreOptionsButton.frame)+20;
}
...
- (void)layoutSubviews
{
    ...
    self.moreOptionsButton.frame = CGRectMake(width - 48*2-25, 20, 48, 48);
    self.deleteButton.frame = CGRectMake(width - 48 -12, 20, 48, 48);
}
...
- (void)setDeleteButtonTitle:(NSString *)deleteButtonTitle
{
    ...
    [self.deleteButton setBackgroundImage:[UIImage imageNamed:self.deleteButtonTitle] forState:UIControlStateNormal];
...
- (void)setMenuOptionsViewHidden:(BOOL)hidden animated:(BOOL)animated completionHandler:(void (^)(void))completionHandler
{
    ...
    [UIView animateWithDuration:(animated) ? self.menuOptionsAnimationDuration : 0.
                     ...
                     animations:^
     {
         self.actualContentView.frame = CGRectMake(frame.origin.x, frame.origin.y, self.actualContentView.frame.size.width, self.actualContentView.frame.size.height);
...
- (void)setMoreOptionsButtonTitle:(NSString *)moreOptionsButtonTitle
{
    ...
    [self.moreOptionsButton setContentMode:UIViewContentModeCenter];
    [self.moreOptionsButton setBackgroundImage:[UIImage imageNamed:self.moreOptionsButtonTitle] forState:UIControlStateNormal];
    [self setNeedsLayout];
}
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer;
{
    ...
    //UIImageView *imgView =  (UIImageView *)[self.actualContentView viewWithTag:111];
    ...
    if ([recognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (recognizer.state == UIGestureRecognizerStateBegan) {
            ...
            if (!self.contextMenuHidden || (velocity.x > 0. || [self.delegate shouldShowMenuOptionsViewInCell:self])) {
                ...
                self.actualContentView.frame = CGRectMake(originX, 0., self.actualContentView.frame.size.width, self.actualContentView.frame.size.height);
...
- (UIButton *)moreOptionsButton
{
    if (!_moreOptionsButton) {
        ...
        _moreOptionsButton = [UIButton buttonWithType:UIButtonTypeCustom];
        _moreOptionsButton.backgroundColor = [UIColor clearColor];//[UIColor lightGrayColor];
...
- (UIButton *)deleteButton
{
    if (self.editable) {
        if (!_deleteButton) {
            ...
            _deleteButton.backgroundColor = [UIColor clearColor];//[UIColor colorWithRed:251./255. green:34./255. blue:38./255. alpha:1.];
